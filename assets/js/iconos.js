$(document).ready(function() {
    ...
    const PLACEHOLDER_ICONO = 'bi-question-square';
    ...
    // Motor reutilizable en utils.js (inicializarCombobox). Al seleccionar/escribir se
    // actualiza la vista previa y el Valor. cerrarListaIconos se mantiene como envoltura
    // porque también se usa fuera del combobox (cambio de tipo y reseteo del formulario).
    const comboIconos = inicializarCombobox({
        input:      '#input_valor_bootstrap',
        lista:      '#lista-iconos-bootstrap',
        chevron:    '#btn-abrir-iconos',
        contenedor: '#combobox-bootstrap',
        max:        MAX_RESULTADOS,
        claseItem:  'd-flex align-items-center gap-2',
        opciones:   function() { return CATALOGO; },
        // El catálogo son strings; se filtra ignorando el prefijo 'bi-'.
        filtrar: function(opciones, q) {
            const t = q.replace(/^bi-/, '');
            return opciones.filter(function(nombre) { return nombre.indexOf(t) !== -1; });
        },
        ...
        onSelect: function(nombre) {
            ...
            previewBootstrap();
            ejecutarValidacionUniversal($inputIcono);   // check verde + autocompleta Nombre
            actualizarValorCrear();
        }
    });
